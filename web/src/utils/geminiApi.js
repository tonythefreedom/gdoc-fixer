...
- 첨부된 Excel 데이터가 있으면 사용자 지시에 따라 해당 데이터를 참조하여 문서를 수정하세요. Excel 데이터는 시트별 CSV 형식으로 제공됩니다.
...
export async function modifyDocumentHtml(currentHtml, instruction, excelContext = null) {
  if (!API_KEY) {
    throw new Error('VITE_GEMINI_API_KEY가 설정되지 않았습니다.');
  }

  // Excel 첨부 데이터가 있으면 프롬프트에 포함
  const excelSection = excelContext
    ? `\n\n── 첨부된 Excel 데이터 (참조용) ──\n${excelContext}\n── Excel 데이터 끝 ──\n`
    : '';
  ...
  if (isImageRelated(instruction)) {
    ...
    if (imageDescriptions.length > 0) {
      ...
      if (generatedImages.length > 0) {
        ...
        const userText = `현재 HTML 문서:\n\n${currentHtml}${excelSection}\n\n사용 가능한 이미지 플레이스홀더:\n${imageInfo}\n\n수정 지시:\n${instruction}`;
        let html = await callProModel(MODIFY_DOCUMENT_WITH_IMAGES_PROMPT, userText);
