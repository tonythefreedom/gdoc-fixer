...
주어진 HTML 문서와 사용자의 수정 지시를 분석하여, **수정이 필요한 부분만** JSON 배열로 반환하세요.
문서가 매우 크므로 전체를 다시 출력하지 마세요. 변경 사항만 정확히 지정하세요.

응답 형식 (JSON 배열만 출력):
[
  {"find": "짧은 고유 텍스트", "replace": "대체할 텍스트"},
  {"find": "다른 고유 텍스트", "replace": "새 내용"}
]

**[중요] find 문자열 최소화 규칙:**
- find는 문서에서 해당 위치를 **고유하게 식별할 수 있는 가장 짧은 텍스트**만 사용하세요.
- HTML 태그와 속성을 모두 포함하지 마세요. 고유한 텍스트 내용만으로 충분합니다.
- 예: find에 "<td class=\\"_s5\\"><p class=\\"_s6\\">검사 결과</p></td>" 대신 "검사 결과</p></td>" 처럼 짧게.
- 주변 텍스트가 고유하다면 태그 없이 순수 텍스트만 사용해도 됩니다.
- find 문자열은 가능한 200자 이내로 유지하세요.

규칙:
- find 값은 원본 HTML에서 **정확히** 존재하는 텍스트여야 합니다.
- replace에는 수정된 결과를 넣으세요.
- 내용을 삭제하려면 replace를 빈 문자열 ""로 설정하세요.
- 내용을 추가하려면 find에 삽입 위치의 기존 HTML을 넣고, replace에 기존 내용 + 추가 내용을 넣으세요.
...
export async function modifyDocumentHtml(currentHtml, instruction, attachments = []) {
  ...
  if (isLargeDoc) {
    ...
    let diffs;
    try {
      diffs = JSON.parse(rawResponse);
      if (!Array.isArray(diffs)) throw new Error('배열이 아닙니다');
    } catch (e) {
      console.error('[modifyDocumentHtml] Diff JSON 파싱 실패:', e.message, rawResponse.slice(0, 200));
      throw new Error('문서 수정 결과 파싱에 실패했습니다. 다시 시도해주세요.');
    }
