 ...
 */
export function buildDesignSystemPromptBlock(designSystemIdOrDefinition) {
  // 내장 프리셋 id 를 받던 함수지만, 사용자가 만든 정의 객체도 그대로 받는다.
  // 커스텀 디자인 시스템도 내장과 똑같은 프롬프트 블록을 얻게 하기 위해서다.
  const ds =
    typeof designSystemIdOrDefinition === 'object' && designSystemIdOrDefinition
      ? designSystemIdOrDefinition
      : getDesignSystem(designSystemIdOrDefinition);
  if (!ds) return '';
...
/**
 * 사용자가 만든 디자인 시스템 정의를 검증하고 빠진 값을 기본값으로 채운다.
 *
 * 팔레트만 주면 나머지(타이포·레이아웃)는 기본 프리셋에서 가져온다. 외부 AI 가
 * 색만 정해 보내도 바로 쓸 수 있게 하려는 것이다.
 *
 * @returns {{ok: true, value: object} | {ok: false, error: string}}
 */
export function validateDesignSystem(input) {
  if (!input || typeof input !== 'object') {
    return { ok: false, error: '디자인 시스템 정의(객체)가 필요합니다.' };
  }
  const name = String(input.name || '').trim();
  if (!name) return { ok: false, error: 'name 이 필요합니다.' };

  const PALETTE_KEYS = ['background', 'surface', 'primary', 'accent', 'text', 'muted', 'divider'];
  const palette = input.palette || {};
  const missing = PALETTE_KEYS.filter((k) => !palette[k]);
  if (missing.length) {
    return { ok: false, error: `palette 에 ${missing.join(', ')} 이(가) 없습니다. 필요한 키: ${PALETTE_KEYS.join(', ')}` };
  }

  // 빠진 영역은 기본 프리셋 값을 쓴다.
  const base = SLIDE_DESIGN_SYSTEMS[0];
  const typography = { ...base.typography, ...(input.typography || {}) };
  typography.weights = { ...base.typography.weights, ...((input.typography || {}).weights || {}) };
  const layout = { ...base.layout, ...(input.layout || {}) };

  return {
    ok: true,
    value: {
      name: name.slice(0, 60),
      nameEn: String(input.nameEn || name).slice(0, 60),
      description: String(input.description || '').slice(0, 300),
      palette: Object.fromEntries(PALETTE_KEYS.map((k) => [k, String(palette[k]).slice(0, 120)])),
      typography,
      layout,
      // promptHint 는 이 디자인의 성격을 문장으로 설명하는 자리다. 없으면 description 으로 대신한다.
      promptHint: String(input.promptHint || input.description || name).slice(0, 4000),
    },
  };
}
